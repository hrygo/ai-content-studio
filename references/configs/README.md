...
VoiceForge 使用 JSON 配置文件控制多角色音频生成的音色、音质和行为。四个文件各司其职，分别对应不同的引擎和使用场景。
...
```bash
# 指定角色库
voiceforge dialogue --source "内容.txt" --roles configs/studio_roles.json -o out.mp3

# MiniMax 引擎独立使用
voiceforge dialogue --source dialogue.txt --roles configs/studio_roles.json -o out.mp3
```
...
```bash
voiceforge dialogue --source "内容.txt" --roles configs/roles.json -o out.mp3
```
