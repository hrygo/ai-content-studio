...
name: voiceforge
description: |
  VoiceForge — 专业级 AI 音频内容创作工具（v2.0.0）。
...
- `protocols/` — 接口协议（TTSEngine、LLMEngine、AudioProcessor）
- `clients/` — API 客户端（MiniMax / Qwen，共享 SSE/流式解析）
- `engines/` — 引擎实现（TTS: MiniMax / Qwen TTS / Qwen Omni；LLM: MiniMax / Qwen）
- `audio/` — FFmpeg 音频处理器
- `use_cases/` — 业务用例（合成、对话、播客、批量）
- `fallback/` — 通用 Fallback 执行器
- `infrastructure/` — CLI、依赖注入容器

---

## 快速开始

安装后即可使用 `voiceforge` CLI：
...
# 一句话跑通（AI 播客）
voiceforge studio --topic "人工智能的未来" -o podcast.mp3

# 对话脚本 TTS
voiceforge dialogue --source dialogue.txt -o dialogue.mp3

# 单文本 TTS
voiceforge synthesize --source "你好世界" -o hello.mp3

# 批量合成
voiceforge batch --segments "你好|cherry,世界|ethan" -o batch.mp3
```
...
### voiceforge synthesize

单文本 TTS 合成（绕过 LLM，直接合成已有文本）。

```bash
voiceforge synthesize --source "待合成文本" -o output.mp3 [OPTIONS]
...
```bash
# MiniMax 指定音色
voiceforge synthesize --source "欢迎收听本期节目" --voice female-tianmei --speed 1.2 -o intro.mp3

# Qwen Omni
voiceforge synthesize --source "这是一段测试音频" --engine qwen_omni --voice cherry -o test.mp3
```
...
```bash
voiceforge dialogue --source DIALOGUE.txt -o output.mp3 [OPTIONS]
...
voiceforge dialogue --source podcast.txt -o podcast.mp3

# 自定义角色音色
voiceforge dialogue --source podcast.txt --roles roles.json -o podcast.mp3

# 加背景音乐
voiceforge dialogue --source podcast.txt --bgm ambient.mp3 -o podcast_bgm.mp3
```
...
```bash
voiceforge studio --topic "播客主题" -o output.mp3 [OPTIONS]
...
```bash
# 最常用：MiniMax 全套
voiceforge studio --topic "人工智能的未来" -o ai_podcast.mp3

# Qwen LLM + MiniMax TTS
voiceforge studio --topic "量子计算原理" --llm qwen --tts minimax -o quantum.mp3

# 加背景音乐
voiceforge studio --topic "产品发布会" --bgm ambient.mp3 -o launch.mp3
```
...
```bash
voiceforge batch --segments "文本1|音色1,文本2|音色2,..." -o output.mp3 [OPTIONS]
...
python3 -c "
import os
from voiceforge.clients import MiniMaxClient
...
| `voiceforge: command not found` | 运行 `pip install -e . --break-system-packages` 重新安装 CLI |
